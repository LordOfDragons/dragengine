...
/**
 * \brief BeOS Native UI Common Dialogs.
 */
class igdeNativeBeOSCommonDialogs{
protected:
	class FileFilter : public BRefFilter{
	private:
		const igdeFilePatternList &pList;
		
	public:
		FileFilter( const igdeFilePatternList &list );
		virtual bool Filter( const entry_ref *ref, BNode *node, struct stat_beos *stat, const char *mimeType );
	};
	
	class OpenSaveDialogHandler : public BHandler{
	private:
		decString pFilename;
		igdeCommonDialogs::eButton pButton;
	...
	public:
		...
		inline const igdeCommonDialogs::eButton GetButton() const{ return pButton; }
	};
