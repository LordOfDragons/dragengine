...
pStrikeThrough( config.strikeThrough )
{
	...
	pNativeFont = new FXFont( FXApp::instance(), pName.GetString(), pSize,
		pBold ? FXFont::Bold : FXFont::Normal,
		pItalic ? FXFont::Italic : FXFont::Straight );
	( ( FXFont* )pNativeFont )->create();
}

igdeFont::~igdeFont(){
	if( pNativeFont ){
		delete ( FXFont* )pNativeFont;
	}
}
...
decPoint igdeFont::TextSize( const char *text ) const{
	...
	FXFont &native = *( ( FXFont* )pNativeFont );
	const int len = strlen( text );
	
	return decPoint( native.getTextWidth( text, len ), native.getTextHeight( text, len ) );
}

deFont *igdeFont::GetEngineFont(){
	if( ! pEngineFont ){
		pEngineFont.TakeOver( igdeFont_FontBuilder( *this ).GenerateFont() );
	}
	return pEngineFont;
}
