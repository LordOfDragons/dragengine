...
public:
	...
	delFileFormat *GetFormatAt( int index ) const;
	
	/** \brief File format is present. */
	bool HasFormat( delFileFormat *format ) const;
	
	/** \brief Index of file format or -1 if absent. */
	int IndexOfFormat( delFileFormat *format ) const;
	
	/** \brief Add file format. */
	void AddFormat( delFileFormat *format );
	
	/** \brief Remove file format. */
	void RemoveFormat( delFileFormat *format );
