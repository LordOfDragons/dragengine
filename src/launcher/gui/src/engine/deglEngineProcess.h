...
#ifndef _DEGLENGINEPROCESS_H_
#define _DEGLENGINEPROCESS_H_


// implementation has to be included either if:
// 
// - this is a posix build
//   under posix forking is supported thus the same source file can contain
//   the implementation together with the declaration
//   
// - this is a windows build and this file is build for the helper executable
//   windows does not provide a fork thus it is required to compile a separate
//   executable. to avoid using multiple files the implementation is only
//   included if this file is build for the helper executable
// 
// the define WITH_IMPLEMENTATION keeps track of this state
#if ! defined OS_W32 || defined OS_W32_HELPER
#define WITH_IMPLEMENTATION 1
#endif


#ifdef WITH_IMPLEMENTATION
#ifdef OS_W32
...
private:
	...
	bool pEngineRunning;
	bool pStopProcess;
	deglEngineProcessRunGame *pRunGame;
...
public:
	...
	#ifdef OS_W32
	deglEngineProcess( HANDLE pipeIn, HANDLE pipeOut, const char *logfile );
	#else
	deglEngineProcess( int pipeIn, int pipeOut, const char *logfile );
	#endif
