...
deglGameXML::deglGameXML( deLogger *logger, const char *loggerSource ) : deglBaseXML( logger, loggerSource ){
}

deglGameXML::~deglGameXML(){
}
...
void deglGameXML::ReadFromFile( decBaseFileReader &reader, deglGame &game ){
...
void deglGameXML::pReadGame( const decXmlElementTag &root, deglGame &game ){
	...
	for( e=0; e<elementCount; e++ ){
		...
		if( strcmp( tag->GetName(), "identifier" ) == 0 ){
			...
			deglFileFormat *fileFormat = NULL;
			try{
				fileFormat = new deglFileFormat;
