...
#include "profile/deglGPDisableModuleVersion.h"
#include "profile/deglGPDisableModuleVersionList.h"
...
// Class deglGameConfigXML
////////////////////////////

// Constructors and Destructors
/////////////////////////////////

deglGameConfigXML::deglGameConfigXML( deLogger* logger, const char* loggingSource, deglGameManager *gameManager ) :
deglSharedConfigXML( logger, loggingSource ){
	if( ! gameManager ){
...
deglGameConfigXML::~deglGameConfigXML(){
}
...
void deglGameConfigXML::ReadFromFile( decBaseFileReader &reader, deglGame &game ){
...
void deglGameConfigXML::WriteToFile( decBaseFileWriter &writer, const deglGame &game ){
...
void deglGameConfigXML::pWriteConfig( decXmlWriter &writer, const deglGame &game ){
...
void deglGameConfigXML::pReadConfig( const decXmlElementTag &root, deglGame &game ){
	...
	for( i=0; i<count; i++ ){
		...
		if( tagName == "customProfile" ){
			...
			if( ! name || ! name[ 0 ] ){
				...
				game.SetActiveProfile( pGameManager->GetProfileList().GetProfileNamed( name ) );
				if( ! game.GetActiveProfile() ){
