...
deglDialogRunGameWith::deglDialogRunGameWith( deglWindowMain *windowMain, deglGame *game, FXWindow *owner ) :
...
pProfile( game->GetActiveProfile() )
{
	...
	pLabIcon = new FXLabel( frameComboBox, "", pWindowMain->GetIconValidSmall(), LABEL_NORMAL, 0, 0, 0, 0, 0, 0, 0, 0 );
	...
	pBtnEdit = guiBuilder.CreateButton( frameComboBox, "Edit Profiles", NULL, this, ID_BTN_EDIT_PROFILES, "Edit Profiles" );
	
	pLabProblem = new FXLabel( frameRight, "", NULL, LABEL_NORMAL | LAYOUT_FILL_X );
	...
	new FXButton( frameLine, "Run Game", NULL, this, ID_ACCEPT, LAYOUT_CENTER_X | FRAME_RAISED | JUSTIFY_NORMAL | ICON_BEFORE_TEXT, 0, 0, 0, 0, 30, 30 );
...
void deglDialogRunGameWith::SetProfile( deglGameProfile *profile ){
	if( pProfile == profile ){
		return;
	}
...
void deglDialogRunGameWith::UpdateGame(){
	...
	pCBProfile->appendItem( "< Default Profile >", NULL );
	...
	deglGameProfile *validateGameProfile = GetProfile();
...
long deglDialogRunGameWith::onCBProfileChanged( FXObject*, FXSelector, void* ){
	if( pCBProfile->getCurrentItem() == -1 ){
		...
		SetProfile( ( deglGameProfile* )pCBProfile->getItemData( pCBProfile->getCurrentItem() ) );
	}
	return 1;
}
...
long deglDialogRunGameWith::onBtnEditProfiles( FXObject*, FXSelector, void* ){
	...
	try{
		deglGameProfile *profile = GetProfile();
		if( ! profile ){
