...
class FragmentGames : Fragment() {
    ...
    fun gameShareLogs(game: Game) {
        ...
        gameInfo.add("android.brand: ${Build.BRAND}")
        gameInfo.add("android.supportedAbis: ${Build.SUPPORTED_ABIS.joinToString(", ")}")
        gameInfo.add("android.device: ${Build.DEVICE}")
        gameInfo.add("android.hardware: ${Build.HARDWARE}")
        gameInfo.add("android.manufacturer: ${Build.MANUFACTURER}")
        gameInfo.add("android.model: ${Build.MODEL}")
        gameInfo.add("android.product: ${Build.PRODUCT}")
        gameInfo.add("android.version.sdkInt: ${Build.VERSION.SDK_INT}")
