...
// Class decDefaultDOctree
///////////////////////////

// Constructors and Destructors
/////////////////////////////////

decDefaultDOctree::decDefaultDOctree( const decDVector &center, const decDVector &halfSize ) : decDOctree( center, halfSize ){
	pElements = NULL;
	pElementCount = 0;
	pElementSize = 0;
}

decDefaultDOctree::~decDefaultDOctree(){
	RemoveAllElements();
...
decDOctree *decDefaultDOctree::CreateOctree( int octant ) const{
	decDVector halfSize = GetHalfSize() * 0.5f;
	const decDVector &center = GetCenter();
	decDOctree *node = ( decDefaultDOctree* )NULL;
	decDVector nc;
	...
	// create child node
	node = ( decDefaultDOctree* )new decDefaultDOctree( nc, halfSize );
	if( ! node ) DETHROW( deeOutOfMemory );
	return node;
}

void decDefaultDOctree::ClearNodeContent(){
	RemoveAllElements();
}

decDefaultDOctree *decDefaultDOctree::InsertIntoTree( void *element, const decDVector &boxCenter, const decDVector &boxHalfSize, int maxDepth ){
	decDOctree *curNode = this;
	decDOctree *nextNode;
	int d;
	...
	( ( decDefaultDOctree* )curNode )->AddElement( element );
	return ( decDefaultDOctree* )curNode;
}
