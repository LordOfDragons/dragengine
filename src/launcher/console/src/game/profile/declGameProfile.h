...
#ifndef _DECLGAMEPROFILE_H_
#define _DECLGAMEPROFILE_H_
 ...
 * @brief Game Profile.
 */
class declGameProfile : public deObject{
private:
	decString pName;
	...
	declGPModuleList pModuleList;
	
	declGPDisableModuleVersionList pDisableModuleVersionList;
...
public:
	...
	declGameProfile( const char *name = "" );
	
	/** \brief Create a copy of a game profile. */
	declGameProfile( const declGameProfile &profile );
	
protected:
	/** Cleans up the game profile. */
	virtual ~declGameProfile();
	/*@}*/
...
public:
	...
	inline declGPModuleList &GetModuleList(){ return pModuleList; }
	inline const declGPModuleList &GetModuleList() const{ return pModuleList; }
	
	/** \brief Retrieves the list of module versions to enable or disable. */
	inline declGPDisableModuleVersionList &GetDisableModuleVersionList(){ return pDisableModuleVersionList; }
	inline const declGPDisableModuleVersionList &GetDisableModuleVersionList() const{ return pDisableModuleVersionList; }
	...
	void CopyFrom( const declGameProfile &profile );
	
	/** Verify profile. */
	void Verify( declLauncher &launcher );
	/** \brief Verify a module. */
	bool VerifyModule( declLauncher &launcher, const char *moduleName, int requiredType ) const;
	/** Activates the profile activating the modules and setting the module properties. */
	void Activate( declLauncher &launcher ) const;
	/*@}*/
};
