...
declGameXML::declGameXML( deLogger *logger, const char *loggerSource ) : declBaseXML( logger, loggerSource ){
}

declGameXML::~declGameXML(){
}
...
void declGameXML::ReadFromFile( decBaseFileReader &reader, declGame &game ){
...
void declGameXML::pReadGame( const decXmlElementTag &root, declGame &game ){
	declFileFormatList &fileFormatList = game.GetFileFormatList();
	...
	for( e=0; e<elementCount; e++ ){
		...
		if( tag ){
			if( strcmp( tag->GetName(), "identifier" ) == 0 ){
				...
				if( pHasAttribute( *tag, "version" ) ){
					game.SetScriptModuleVersion( pGetAttributeString( *tag, "version" ) );
				}
				...
				declFileFormat *fileFormat = NULL;
				try{
					fileFormat = new declFileFormat;
