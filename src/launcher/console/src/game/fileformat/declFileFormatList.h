...
public:
	...
	declFileFormat *GetAt( int index ) const;
	/** Determines if a file format exists. */
	bool Has( declFileFormat *format ) const;
	/** Retrieves the index of a file format or -1 if not found. */
	int IndexOf( declFileFormat *format ) const;
	/** Adds a file format. */
	void Add( declFileFormat *format );
	/** Removes a file format. */
	void Remove( declFileFormat *format );
