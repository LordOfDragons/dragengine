...
from .de_helpers import ActionFCurvesBuilder, FCurveBuilder, ActionSlotHelper
	...
	def readMoves(self, f):
		...
		for i in range(moveCount):
			...
			if self.import_mode == '0' or moveName == self.import_move:
				...
				ao = self.armature.object
				if not ao.animation_data:
					ao.animation_data_create()
				
				ash = ActionSlotHelper(ao)
				ash.slot_id = ao.name
				ash.slot_type = 'OBJECT'
				
				ao.animation_data.action = action
				if ash.has_slots:
					ao.animation_data.action_slot = ash.ensure_slot(action)
				...
				for moveBone in moveBones:
					...
					builder = ActionFCurvesBuilder(ash, action, boneName)
				...
				if self.mesh:
					...
					ash.slot_id = self.mesh.mesh.name
					ash.slot_type = 'KEY'
					
					if not ash.has_slots:
					...
					if ash.has_slots:
						shapeKeys.animation_data.action_slot = ash.ensure_slot(action)
					...
					for moveVps in moveVpSets:
						...
						fcurve = FCurveBuilder(ash, action, "key_blocks[\"{}\"].value".format(vpsName))
