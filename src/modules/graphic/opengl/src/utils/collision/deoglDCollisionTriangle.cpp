...
deoglDCollisionTriangle::deoglDCollisionTriangle(){
}

deoglDCollisionTriangle::deoglDCollisionTriangle( const decDVector &corner1, const decDVector &corner2, const decDVector &corner3 ){
	SetCorners( corner1, corner2, corner3 );
}

deoglDCollisionTriangle::deoglDCollisionTriangle( const decDVector &corner1, const decDVector &corner2, const decDVector &corner3, const decDVector &normal ){
...
bool deoglDCollisionTriangle::VolumeHitsVolume( deoglDCollisionVolume *volume ){
	return volume->TriangleHitsVolume( this );
}

double deoglDCollisionTriangle::VolumeMoveHitsVolume( deoglDCollisionVolume *volume, const decDVector &displacement, decDVector *normal ){
...
bool deoglDCollisionTriangle::SphereHitsVolume( deoglDCollisionSphere *sphere ){
	return SphereHitsTriangle( sphere );
}

bool deoglDCollisionTriangle::CylinderHitsVolume( deoglDCollisionCylinder *cylinder ){
	return CylinderHitsTriangle( cylinder );
}

bool deoglDCollisionTriangle::CapsuleHitsVolume( deoglDCollisionCapsule *capsule ){
	return CapsuleHitsTriangle( capsule );
}

bool deoglDCollisionTriangle::BoxHitsVolume( deoglDCollisionBox *box ){
	return BoxHitsTriangle( box );
}

bool deoglDCollisionTriangle::TriangleHitsVolume( deoglDCollisionTriangle *triangle ){
	return TriangleHitsTriangle( triangle );
}

bool deoglDCollisionTriangle::FrustumHitsVolume( deoglDCollisionFrustum *frustum ){
	return frustum->TriangleHitsFrustum( this );
}



double deoglDCollisionTriangle::SphereMoveHitsVolume( deoglDCollisionSphere *sphere, const decDVector &displacement, decDVector *normal ){
	return SphereMoveHitsTriangle( sphere, displacement, normal );
}

double deoglDCollisionTriangle::CylinderMoveHitsVolume( deoglDCollisionCylinder *cylinder, const decDVector &displacement, decDVector *normal ){
	return CylinderMoveHitsTriangle( cylinder, displacement, normal );
}

double deoglDCollisionTriangle::CapsuleMoveHitsVolume( deoglDCollisionCapsule *capsule, const decDVector &displacement, decDVector *normal ){
	return CapsuleMoveHitsTriangle( capsule, displacement, normal );
}

double deoglDCollisionTriangle::BoxMoveHitsVolume( deoglDCollisionBox *box, const decDVector &displacement, decDVector *normal ){
	return BoxMoveHitsTriangle( box, displacement, normal );
}

double deoglDCollisionTriangle::TriangleMoveHitsVolume( deoglDCollisionTriangle *triangle, const decDVector &displacement, decDVector *normal ){
	return TriangleMoveHitsTriangle( triangle, displacement, normal );
}

double deoglDCollisionTriangle::FrustumMoveHitsVolume( deoglDCollisionFrustum *frustum, const decDVector &displacement, decDVector *normal ){
	double distance = frustum->TriangleMoveHitsFrustum( this, -displacement, normal );
	if( normal ) normal->Negate();
	return distance;
}

double deoglDCollisionTriangle::PointMoveHitsVolume( const decDVector &point, const decDVector &displacement, decDVector *normal ){
	decDVector hitPoint;
	...
	if( dot < -0.00001 || dot > 0.00001 ){
		...
		if( lambda < 0 || lambda > 1 ){
			...
			if( deoglDCollisionDetection::PointInTriangle( pCorners[ 0 ], pCorners[ 1 ],
			pCorners[ 2 ], hitPoint ) ){
				if( normal ){
...
void deoglDCollisionTriangle::GetEnclosingSphere( deoglDCollisionSphere *sphere ){
...
void deoglDCollisionTriangle::GetEnclosingBox( deoglDCollisionBox *box ){
...
bool deoglDCollisionTriangle::IsPointInside( const decDVector &point ){
	return false; // triangles are infinitesimally thin
}

decDVector deoglDCollisionTriangle::ClosestPointTo( const decDVector &point ){
	decDVector hitPoint = point - pNormal * ( point * pNormal - pDist );
	if( deoglDCollisionDetection::PointInTriangle( pCorners[ 0 ], pCorners[ 1 ], pCorners[ 2 ], hitPoint ) ){
		return hitPoint;
	}else{
		return deoglDCollisionDetection::ClosestPointOnTriangleEdge( pCorners[ 0 ], pCorners[ 1 ], pCorners[ 2 ], hitPoint );
	}
}
...
void deoglDCollisionTriangle::Visit( deoglDCollisionVolumeVisitor *visitor ){
...
bool deoglDCollisionTriangle::SphereHitsTriangle( deoglDCollisionSphere *sphere ){
	...
	if( ! deoglDCollisionDetection::PointInTriangle( pCorners[ 0 ], pCorners[ 1 ], pCorners[ 2 ], hitPoint ) ){
		hitPoint = deoglDCollisionDetection::ClosestPointOnTriangleEdge( pCorners[ 0 ], pCorners[ 1 ], pCorners[ 2 ], hitPoint );
	}
...
bool deoglDCollisionTriangle::CylinderHitsTriangle( deoglDCollisionCylinder *cylinder ){
	return false;
}

bool deoglDCollisionTriangle::CapsuleHitsTriangle( deoglDCollisionCapsule *capsule ){
	return false;
}

bool deoglDCollisionTriangle::BoxHitsTriangle( deoglDCollisionBox *box ){
...
bool deoglDCollisionTriangle::TriangleHitsTriangle( deoglDCollisionTriangle *triangle ){
	return false;
}



double deoglDCollisionTriangle::SphereMoveHitsTriangle( deoglDCollisionSphere *sphere, const decDVector &displacement, decDVector *normal ){
	const decDVector &sc = sphere->GetCenter();
...
double deoglDCollisionTriangle::CylinderMoveHitsTriangle( deoglDCollisionCylinder *cylinder, const decDVector &displacement, decDVector *normal ){
	return 1;
}

double deoglDCollisionTriangle::CapsuleMoveHitsTriangle( deoglDCollisionCapsule *capsule, const decDVector &displacement, decDVector *normal ){
	return 1;
}
...
double deoglDCollisionTriangle::BoxMoveHitsTriangle( deoglDCollisionBox *box, const decDVector &displacement, decDVector *normal ){
	const decDVector &bhs = box->GetHalfSize();
	const decDVector &bc = box->GetCenter();
	...
	if( normal ){
		...
		if( hitAtStart ){
			...
			if( deoglDCollisionDetection::PointInTriangle( pCorners[ 0 ], pCorners[ 1 ], pCorners[ 2 ], projPoint ) ){
				normal->Set( pNormal );
			}else{
				projPoint = deoglDCollisionDetection::ClosestPointOnTriangleEdge( pCorners[ 0 ],
...
double deoglDCollisionTriangle::TriangleMoveHitsTriangle( deoglDCollisionTriangle *triangle, const decDVector &displacement, decDVector *normal ){
	return 1;
}
...
void deoglDCollisionTriangle::SetCorners( const decDVector &corner1, const decDVector &corner2, const decDVector &corner3 ){
	decDVector newNormal = ( corner2 - corner1 ) % ( corner3 - corner2 );
	newNormal.Normalize();
	SetCorners( corner1, corner2, corner3, newNormal );
}

void deoglDCollisionTriangle::SetCorners( const decDVector &corner1, const decDVector &corner2, const decDVector &corner3, const decDVector &normal ){
	pCorners[ 0 ] = corner1;
	pCorners[ 1 ] = corner2;
	pCorners[ 2 ] = corner3;
