...
void main(void){
	...
	if(radius > vRimThickness){
		...
		if(angle < 0.35 || angle > 0.65){
			...
			outColor.rgb = mix(colorEnabled, colorDisabled, bvec3((flags & gipfDisabled) == gipfDisabled));
			
		}else{
			const vec3 colorOn = vec3(0.5, 0, 0.5);
			const vec3 colorOff = vec3(0, 0, 0);
			outColor.rgb = mix(colorOff, colorOn, bvec3((flags & gipfNearGeometry) == gipfNearGeometry));
		}
