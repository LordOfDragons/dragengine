...
float depthToZ( in float depth, in int layer ){
	...
	return divisor != 0.0 ? pDepthToPosition[ layer ].x / divisor : pCameraViewDistance;
}

float depthToZ( ARG_SAMP_HIGHP sampler2DArray samplerDepth, in ivec3 texCoord, in int layer ){
	return depthToZ( sampleDepth( samplerDepth, texCoord ), layer );
}

float depthToZ( ARG_SAMP_HIGHP sampler2DArray samplerDepth, in vec3 texCoord, in int layer ){
	return depthToZ( sampleDepth( samplerDepth, texCoord ), layer );
}
...
vec3 depthToPositionVolume( ARG_SAMP_HIGHP sampler2DArray samplerDepth, in ivec3 texCoord, in vec3 volumePosition, in int layer ){
	return depthToPositionVolume( sampleDepth( samplerDepth, texCoord ), volumePosition, layer );
}

vec3 depthToPosition( ARG_SAMP_HIGHP sampler2DArray samplerDepth, in ivec3 texCoord, in vec2 screenCoord, in int layer ){
	return depthToPosition( sampleDepth( samplerDepth, texCoord ), screenCoord, layer );
}

vec3 depthToPosition( ARG_SAMP_HIGHP sampler2DArray samplerDepth, in vec3 texCoord, in vec2 screenCoord, in int layer ){
	return depthToPosition( sampleDepth( samplerDepth, texCoord ), screenCoord, layer );
}

vec3 depthToPositionLod0( ARG_SAMP_HIGHP sampler2DArray samplerDepth, in vec3 texCoord, in vec2 screenCoord, in int layer ){
	return depthToPosition( sampleDepth( samplerDepth, texCoord, 0.0 ), screenCoord, layer );
}
