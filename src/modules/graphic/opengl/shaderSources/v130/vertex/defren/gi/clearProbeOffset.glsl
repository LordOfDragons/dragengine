...
void main( void ){
	...
	vec2 scale = vec2( 1 ) / vec2( pGIGridProbeCount.x * pGIGridProbeCount.y, pGIGridProbeCount.z );
	#define realSize scale
	vec2 realOffset = vec2( texCoord ) * scale * vec2( 2 ) + realSize + vec2( -1 );
	
	gl_Position = vec4( vec3( inPosition * realSize + realOffset, 0 ), 1 );
}
