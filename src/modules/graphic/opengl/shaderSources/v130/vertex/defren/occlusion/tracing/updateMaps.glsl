...
void main( void ){
	...
	#endif
	int mapProbeStride = mapProbeSize + 2;
	
	int groupOffset = pGridProbeCount.x * probeGrid.y + probeGrid.x;
	vOffset.x = groupOffset * mapProbeStride + 2;
	vOffset.y = probeGrid.z * mapProbeStride + 2;
	
	vec2 realSize = vec2( mapProbeSize ) * mapProbeScale;
