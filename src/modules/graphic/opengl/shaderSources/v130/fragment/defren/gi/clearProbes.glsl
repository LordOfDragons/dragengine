...
void main( void ){
	...
	#ifdef MAP_IRRADIANCE
		tc /= ivec2( pGIIrradianceMapSize + 2 );
	#else
		tc /= ivec2( pGIDistanceMapSize + 2 );
	#endif
	
	// discard if inside image border (or rather outside probes including their border)
	if( any( lessThan( tc, ivec2( 0 ) ) )
	|| any( greaterThanEqual( tc, ivec2( pGIGridProbeCount.x * pGIGridProbeCount.y, pGIGridProbeCount.z ) ) ) ){
		discard;
	}
	...
	int probeIndex = pGIGridProbeCount.x * pGIGridProbeCount.z * probeCoord.y
		+ pGIGridProbeCount.x * probeCoord.z + probeCoord.x;
