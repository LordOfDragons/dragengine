...
deoalTimeHistory::deoalTimeHistory( int initialSize, int cutExtremeValues ) :
pEntries( NULL ),
pSorted( NULL ),
pSize( 0 ),
pHead( 0 ),
...
deoalTimeHistory::~deoalTimeHistory(){
	if( pSorted ){
		delete [] pSorted;
	}
	if( pEntries ){
...
void deoalTimeHistory::SetSize( int size ){
	if( size == pSize ){
		return;
	}
...
float deoalTimeHistory::GetAt( int position ) const{
...
void deoalTimeHistory::Add( float time ){
	pEntries[ pTail ] = time;
...
void deoalTimeHistory::SetCutExtremeValues( int cutExtremeValues ){
...
decString deoalTimeHistory::DebugPrint() const{
...
void deoalTimeHistory::pCalculateMetrics(){
	if( pCount == 0 ){
		// no entries
		pHasMetrics = false;
		pMinimum = 0.0f;
