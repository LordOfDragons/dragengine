...
DEBUG_RESET_TIMERS;
	...
	// move times
	const float ltime = pMove1->GetPlaytime() * decMath::clamp( pTargetLeadingMoveTime.GetValue(
		GetInstance(), pAnimationDifference.GetLeadingMoveTime()
			/ decMath::max( pMove1->GetPlaytime(), 0.01f ) ), 0.0f, 1.0f );
	
	const float rtime = pMove2->GetPlaytime() * decMath::clamp( pTargetReferenceMoveTime.GetValue(
		GetInstance(), pAnimationDifference.GetReferenceMoveTime()
			/ decMath::max( pMove2->GetPlaytime(), 0.01f ) ), 0.0f, 1.0f );
