...
$TargetDir = Join-Path -Path $OutputDir -ChildPath "Distribute\Dragengine\Application\@ProgramFiles\Dragengine\Data\modules\rig\fbxrig\$Version"

Write-Host "FBXRig Module: Copy Module to '$TargetDir'"

$Library = Join-Path -Path $OutputDir -ChildPath "rigfbx.dll"
...
$FilePath = Join-Path -Path $OutputDir -ChildPath "rigfbx.pdb"
